...
import { Database, Cpu, Activity, CheckCircle2, ArrowRight, Sparkles, Layers } from 'lucide-react';
...
export const DecisionFlowVisualizer = () => {
  ...
  return (
    <div className="glass-card rounded-3xl p-6 md:p-10 border border-purple-500/30 relative overflow-hidden">
      {/* Background glow */}
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl border text-left transition-all duration-300 flex items-center gap-3.5 cursor-pointer ${
                isSelected
                  ? 'bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-blue-500/20 border-purple-500 text-slate-900 shadow-xl shadow-purple-500/15 scale-[1.02]'
                  : 'bg-white/60 border-purple-500/20 text-slate-700 hover:text-purple-600 hover:border-purple-400'
              }`}
            >
              <div className={`p-2.5 rounded-xl border ${isSelected ? 'bg-gradient-to-r from-pink-500 to-purple-600 text-white border-purple-400' : 'bg-white border-purple-500/20 text-purple-600'}`}>
        ...
        {steps.map((s) => {
          ...
          return (
            <motion.div
              ...
              className="p-6 md:p-8 rounded-3xl bg-white/80 backdrop-blur-xl border border-purple-500/30 grid grid-cols-1 md:grid-cols-3 gap-6 items-center shadow-xl"
            >
              <div className="md:col-span-2 space-y-3">
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1 rounded-xl bg-purple-500/15 text-purple-700 text-xs font-mono font-black border border-purple-400/30">
                    STAGE 0{s.id}
                  </span>
                ...
                <div className="pt-2 flex items-center gap-4 text-xs font-extrabold text-purple-600">
                  <span className="flex items-center gap-1.5"><Layers className="w-4 h-4" /> Automatic Vectorization</span>
                  <span className="flex items-center gap-1.5"><Activity className="w-4 h-4 text-blue-600" /> Real-time Telemetry</span>
                </div>
              </div>

              {/* Animated Graph / Circuit Mockup */}
              <div className="p-6 rounded-3xl bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-500/20 flex flex-col items-center justify-center text-center gap-3">
                <div className="w-16 h-16 rounded-full bg-gradient-to-tr from-pink-500 via-purple-500 to-blue-500 p-0.5 shadow-xl shadow-purple-500/30 animate-pulse">
                  <div className="w-full h-full bg-white rounded-full flex items-center justify-center">
                    <Icon className="w-8 h-8 text-purple-600" />
                  </div>
                </div>
                <p className="text-xs font-black text-slate-900">Quantum Node Simulation</p>
